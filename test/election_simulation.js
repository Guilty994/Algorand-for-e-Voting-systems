...
import { computeResults } from "../src/result_audit/compute_results.js";
...
export const main = async () => {
    ...
    const voterIdentityCode = ['0000', '1111', '2222'] // this codes are generated by EA and sent to the voters
    ...
    await preRegistrationRequest(returnedIDs, voterAccount1, 'electionauthoritytest+ea@gmail.com', votersMails[0], voterIdentityCode[0], client);
    console.groupEnd("VOTER 1")
    console.group(chalk.bgCyanBright("VOTER 2"))
    await preRegistrationRequest(returnedIDs, voterAccount2, 'electionauthoritytest+ea@gmail.com', votersMails[1], voterIdentityCode[1], client);
    console.groupEnd("VOTER 2")
    console.group(chalk.bgCyanBright("VOTER 3"))
    await preRegistrationRequest(returnedIDs, voterAccount3, 'electionauthoritytest+ea@gmail.com', votersMails[2], voterIdentityCode[2], client);
    console.groupEnd("VOTER 3")
    console.groupEnd("PRE-REGISTRATION")

    

    // Simulate confirm identity (after RA confirm the identity)
    console.group(chalk.bgGreenBright("IDENTITY CONFIRMATION"))
    console.group(chalk.bgCyanBright("VOTER 1"))
    console.log("Identity was confirmed by RA for "+voterAccount1.addr)
    await acceptRegistration(electAuthAccount, voterAccount1.addr, returnedIDs.appID, returnedIDs.ballotID, votersMails[0], client);
    console.groupEnd("VOTER 1")
    console.group(chalk.bgCyanBright("VOTER 2"))
    console.log("Identity was confirmed by RA for "+voterAccount2.addr)
    await acceptRegistration(electAuthAccount, voterAccount2.addr, returnedIDs.appID, returnedIDs.ballotID, votersMails[1], client);
    console.groupEnd("VOTER 2")
    console.group(chalk.bgCyanBright("VOTER 3"))
    console.log("Identity was confirmed by RA for "+voterAccount3.addr)
